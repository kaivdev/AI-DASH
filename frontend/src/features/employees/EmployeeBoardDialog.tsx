        ...
        <div className={`w-full max-w-5xl rounded-lg border bg-background shadow-xl transition-all duration-200 ${open ? 'opacity-100 scale-100' : 'opacity-0 scale-95'}`}>
          ...
          <div className="p-5 space-y-4">
            ...
            <div className="p-3 border rounded bg-muted/5 grid grid-cols-1 md:grid-cols-5 gap-2">
              ...
              <select className="h-9 px-3 rounded border bg-background text-sm" value={posFilter} onChange={(e) => setPosFilter(e.target.value)}>
              ...
              {filtered.map((e) => (
                <div key={e.id} className="px-3 py-3 border-b last:border-b-0">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <button className="text-xs text-red-600" onClick={() => onRemove(e.id)}>Удалить</button>
                  </div>

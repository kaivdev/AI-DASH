...
# Публичный URL вебхука (например, через Cloudflare Tunnel/Ngrok). Должен вести на /api/telegram/webhook
TELEGRAM_WEBHOOK_URL=https://your.public.domain/api/telegram/webhook
# Секрет для проверки заголовка X-Telegram-Bot-Api-Secret-Token
TELEGRAM_WEBHOOK_SECRET=changeme-secret
```
...
- Привязка чата: в Telegram напишите вашему боту команду `/start`, затем `/link email@example.com` — где email совпадает с полем email сотрудника.
- Создайте задачу с назначением на этого сотрудника — бот пришлёт уведомление.

Если деплой локальный, настройте проксирование (Cloudflare Tunnel/Ngrok) и укажите TELEGRAM_WEBHOOK_URL.
...
| `TELEGRAM_WEBHOOK_URL` | Публичный URL вебхука | — |
| `TELEGRAM_WEBHOOK_SECRET` | Секрет заголовка для верификации | `changeme-secret` |
...
По умолчанию API использует локальный Ollama/LM Studio совместимый сервер (настройки OLLAMA_URL, OLLAMA_MODEL). Также поддерживается OpenRouter на основе персонального токена пользователя:

- Пользователь может указать OpenRouter API Key в своём профиле (в веб‑интерфейсе Аккаунт). Ключ хранится в таблице user_profiles.openrouter_api_key и не возвращается клиенту.
- Если ключ установлен, вызовы /api/ai/command и /api/ai/chat будут маршрутизированы через OpenRouter (модель OPENROUTER_MODEL, по умолчанию openai/gpt-4o-mini). Иначе используется локальный Ollama.